...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'PermID');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!apiKey) {
    throw new Error(
      'PermID is BYO-only. Pass ?_apiKey=<your_token> on the gateway URL. Register at https://permid.org/signin (LSEG account required — free 5,000 req/day).',
    );
  }
  switch (name) {
    case 'search_entities':
      return searchEntities(apiKey, { ...args, query: reqStr(args, 'query', '"Apple Inc" or ticker "AAPL.OQ"') });
    case 'get_entity':
      return getEntity(apiKey, reqStr(args, 'permid', '"4295905573" (Apple Inc PermID)'));
    default:
